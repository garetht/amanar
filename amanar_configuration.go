...
type Configurables struct {
	...
	TemplateDatasources       []TemplateDatasource       `json:"template_datasources",yaml:"template_datasources"`       // Fills credentials into a provided template string or template file and prints the result; to stdout. Unlike the other datasources, templates are generated anew each time and do; not attempt to find previously existing keys to modify.
}
...
type TemplateDatasource struct {
	Template     *string `json:"template,omitempty",yaml:"template,omitempty"`     // A template string that will be filled in with credentials.
	TemplatePath *string `json:"template_path,omitempty",yaml:"template_path,omitempty"`// The path to the template file that will be filled in with credentials.
}
